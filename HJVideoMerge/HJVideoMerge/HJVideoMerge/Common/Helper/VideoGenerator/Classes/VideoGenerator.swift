...
public class VideoGenerator: NSObject {
  ...
  public static var fileName = "movie111"
  ...
  public static var videoDurationInSeconds: Double = 0 {
    didSet {
      videoDurationInSeconds = Double(CMTime(seconds: videoDurationInSeconds, preferredTimescale: 125).seconds)
    }
  }
   ...
   */
  open func generate(withImages _images: [UIImage], andType _type: VideoGeneratorType, _ progress: @escaping ((Progress) -> Void), outcome: @escaping (Result<URL, Error>) -> Void) {
    
    let dispatchQueueGenerate = DispatchQueue(label: "generate", qos: .background)
    
    dispatchQueueGenerate.async { [weak self] in
      
      VideoGenerator.current.setup(withImages: _images, andType: _type)
                ...
                if self?.audioURLs.isEmpty == true {
                  if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
                    ...
                    PHPhotoLibrary.shared().performChanges({
                        PHAssetChangeRequest.creationRequestForAssetFromVideo(atFileURL: newPath)
                    }) { saved, error in
                                if saved {
                                    let fetchOptions = PHFetchOptions()
                                    fetchOptions.sortDescriptors = [NSSortDescriptor(key: VideoGenerator.fileName, ascending: true)]
  ...
  open class func mergeMovies(videoURLs: [URL], outcome: @escaping (Result<URL, Error>) -> Void) {
    let acceptableVideoExtensions = ["mov", "mp4", "m4v","M4V"]
    ...
    if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
      ...
      completeMoviePath = URL(fileURLWithPath: documentsPath).appendingPathComponent("\(VideoGenerator.fileName).mp4")
        ...
        if let exportSession = AVAssetExportSession(asset: composition, presetName: presetName ?? AVAssetExportPresetHighestQuality) {
          ...
          exportSession.exportAsynchronously(completionHandler: {
            ...
            default:
              ...
              DispatchQueue.main.async {
                PHPhotoLibrary.shared().performChanges({
                    PHAssetChangeRequest.creationRequestForAssetFromVideo(atFileURL: completeMoviePath)
                              }) { saved, error in
                                          if saved {
                                              let fetchOptions = PHFetchOptions()
                                              fetchOptions.sortDescriptors = [NSSortDescriptor(key: VideoGenerator.fileName, ascending: true)]
   ...
   */
  fileprivate func setup(withImages _images: [UIImage], andType _type: VideoGeneratorType) {
    images = []
    audioURLs = []
    audioDurations = []
    duration = 0.0
